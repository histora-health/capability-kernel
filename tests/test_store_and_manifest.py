...
def test_the_two_namings_describe_the_same_world():
    """Only the identifiers differ, so any measured difference is the names.

    If the schemes diverged in structure — one signed study versus two, a
    different file count — a workspace difference between them would be
    uninterpretable.
    """
    from capability_kernel.manifest import surface_size

    opaque, semantic = demo_store("opaque"), demo_store("semantic")
    assert surface_size(opaque) == surface_size(semantic)
    assert len(opaque.folders) == len(semantic.folders)
    assert len(opaque.files) == len(semantic.files)
    assert [f.name for f in opaque.files] == [f.name for f in semantic.files]
    assert [f.signed for f in opaque.folders] == [f.signed for f in semantic.folders]


def test_semantic_ids_still_close_the_signed_study():
    """The mechanism must not depend on the naming scheme."""
    from capability_kernel.manifest import MANIFEST, VIRTUAL, legal_values

    store = demo_store("semantic")
    for method in MANIFEST:
        if method in VIRTUAL:
            continue
        for arg in MANIFEST[method].args:
            values = legal_values(store, method, arg) or []
            assert "hygiene_study_signed" not in values
            assert "periodontal_chart" not in values


def test_an_unknown_naming_is_refused():
    with pytest.raises(ValueError, match="naming must be"):
        demo_store("clinical")
